...
green "域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /usr/local/ssl文件夹内" 
yellow "公钥文件crt路径如下，可直接复制"
green "/usr/local/ssl/cert.crt"
yellow "密钥文件key路径如下，可直接复制"
green "/usr/local/ssl/private.key"
echo $ym > /usr/local/ssl/ca.log
...
installCA(){
bash ~/.acme.sh/acme.sh --install-cert -d ${ym} --key-file /usr/local/ssl/private.key --fullchain-file /usr/local/ssl/cert.crt --ecc
}
...
yellow " 公钥文件crt保存路径：/usr/local/ssl/cert.crt"
yellow " 密钥文件key保存路径：/usr/local/ssl/private.key"
echo
red "========================================================================="
acmeshow
